...
void _registerResponsiveTests() {
  ...
  testWidgets(
    ...
    (tester) => _survivesUnboundedWidth(
      tester,
      LinagoraEventCardLayout.regular,
    ),
  );
  testWidgets(
    'shrink-wraps an adaptive card in unbounded width',
    (tester) => _survivesUnboundedWidth(
      tester,
      LinagoraEventCardLayout.adaptive,
    ),
  );
  testWidgets(
    'shrink-wraps a compact card in unbounded width',
    (tester) => _survivesUnboundedWidth(
      tester,
...
Future<void> _survivesUnboundedWidth(
  WidgetTester tester,
        ...
        body: Row(
          children: [
            LinagoraEventCard(
              ...
              date: _date,
              title: 'Event',
              conference: const LinagoraEventConference(
                join: LinagoraEventAction(label: 'Join'),
              ),
              details: const [
                LinagoraEventDetail(
                  label: 'When',
                  values: [LinagoraEventValue('Today')],
                ),
              ],
              calendarAction: const LinagoraEventAction(label: 'Calendar'),
            ),
          ],
        ),
      ),
  ...
  expect(tester.takeException(), isNull);
  expect(
    tester.getSize(find.byType(LinagoraEventCard)).width,
    lessThan(1200),
    reason: 'an unbounded card must hug its content instead of filling the row',
  );
  expect(
...
Future<void> _localisesCopyAction(WidgetTester tester) async {
  ...
  try {
      ...
      _host(
        LinagoraEventCard(
          conference: LinagoraEventConference(
            onCopyLink: () {},
            copyTooltip: 'Copier',
            copySemanticLabel: 'Copier le lien de conférence',
          ),
        ),
      ),
    );
    ...
    expect(
      tester.getSemantics(
        find.bySemanticsLabel('Copier le lien de conférence'),
      ),
      matchesSemantics(
        label: 'Copier le lien de conférence',
        isButton: true,
...
void _rejectsInvalidCardMetrics() {
  for (final value in [-1.0, double.infinity]) {
    final invalidCards = <Object Function()>[
      () => LinagoraEventCard(borderRadius: value),
      () => LinagoraEventCard(compactBreakpoint: value),
      () => LinagoraEventCard(prefixWidth: value),
    ...
    for (final createCard in invalidCards) {
      expect(
        createCard,
        throwsAssertionError,
        reason: 'Invalid card metric $value must be rejected',
      );
    }
  }
...
void _invalidExpansion() {
  for (final valueSpacing in [-1.0, double.infinity]) {
    expect(
      () => LinagoraEventDetail(valueSpacing: valueSpacing),
      throwsAssertionError,
      reason: 'Invalid value spacing $valueSpacing must be rejected',
    );
  }
  expect(
    () => const LinagoraEventDetail(valueSpacing: 0),
    returnsNormally,
  );
  expect(
