...
class LinagoraEventCard extends StatelessWidget {
  ...
  const LinagoraEventCard({
    ...
    this.prefixWidth = LinagoraEventInfoRow.defaultPrefixWidth,
         ...
         'Border radius must be finite and non-negative',
       ),
       assert(
         compactBreakpoint >= 0 && compactBreakpoint < double.infinity,
         'Compact breakpoint must be finite and non-negative',
       ),
       assert(
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final compact = switch (layout) {
          LinagoraEventCardLayout.regular => false,
          LinagoraEventCardLayout.compact => true,
          LinagoraEventCardLayout.adaptive =>
            width.isFinite && width < compactBreakpoint,
        };

        return _buildCard(
          compact: compact,
          canExpand: constraints.hasBoundedWidth,
        );
      },
    );
  }

  Widget _buildCard({required bool compact, required bool canExpand}) {
    ...
    return Container(
      ...
      child: LinagoraEventInfoTypeScale(
        ...
        child: compact
            ? _buildCompactBody(metrics, canExpand: canExpand)
            : _buildRegularBody(metrics, canExpand: canExpand),
      ),
    );
  }
  ...
  Widget _buildRegularBody(
    ...
    final body = Column(
      ...
      crossAxisAlignment: canExpand
          ? CrossAxisAlignment.stretch
          : CrossAxisAlignment.start,
      spacing: metrics.sectionSpacing,
      children: [
        ..._sections([
          _buildRegularHeader(metrics, canExpand: canExpand),
          _buildDetails(metrics, compact: false, canExpand: canExpand),
        ]),
      ],
    );
  ...
  Widget _buildConferenceActions(LinagoraEventConference conference) {
    ...
    return EventConferenceActions(
      ...
      copyButtonSemanticLabel: conference.copySemanticLabel ?? 'Copy link',
    );
  }
