...
class EventConferenceActions extends StatelessWidget {
  ...
  static const Color defaultDisabledVideoButtonBackground =
      LinagoraButton.disabledContainerColor;
  static const Color defaultDisabledVideoButtonForeground =
  ...
  static const double defaultDisabledCopyIconOpacity = 0.38;
  ...
  /// Optional visual tooltip for the copy-link icon button.
  final String? copyButtonTooltip;

  /// Accessible name for the copy-link icon button, independent of whether a
  /// visual tooltip is shown.
  final String copyButtonSemanticLabel;
  ...
  const EventConferenceActions({
    ...
    this.copyButtonSemanticLabel = 'Copy link',
    this.videoButtonStyle,
    this.copyIconColor = defaultCopyIconColor,
    this.copyOverlayColor = defaultCopyOverlayColor,
  }) : assert(
         videoIconSize > 0 && videoIconSize < double.infinity,
         'Video icon size must be finite and positive',
       ),
       assert(
         copyIconSize > 0 && copyIconSize < double.infinity,
         'Copy icon size must be finite and positive',
       ),
       assert(
         copyButtonSemanticLabel != '',
         'Copy button semantic label must not be empty',
       );

  @override
  Widget build(BuildContext context) {
    assert(
      copyButtonSemanticLabel.trim().isNotEmpty,
      'Copy button semantic label must not be blank',
    );
  ...
  Widget _buildCopyButton() {
    return Semantics(
      label: copyButtonSemanticLabel,
      button: true,
      enabled: onCopyLinkPressed != null,
      onTap: onCopyLinkPressed,
  ...
  Widget? get _copyIconWidget {
    ...
    if (customIcon != null) return _copyWidgetForState(customIcon);
    if (copyIcon != null) return null;
    return _copyWidgetForState(
      SvgPicture.asset(
  ...
  Widget _copyWidgetForState(Widget icon) {
    if (onCopyLinkPressed != null) return icon;
    return Opacity(opacity: defaultDisabledCopyIconOpacity, child: icon);
  }
  ...
  static final ButtonStyle _defaultVideoButtonStyle = ButtonStyle(
    backgroundColor: WidgetStateProperty.resolveWith(
      (states) => states.contains(WidgetState.disabled)
          ? defaultDisabledVideoButtonBackground
          : defaultVideoButtonBackground,
    ),
    foregroundColor: WidgetStateProperty.resolveWith(
      (states) => states.contains(WidgetState.disabled)
          ? defaultDisabledVideoButtonForeground
          : defaultVideoButtonForeground,
    ),
