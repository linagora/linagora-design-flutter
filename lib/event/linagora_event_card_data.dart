...
class LinagoraEventDate {
  /// An already-localised, non-blank month label.
  final String month;
...
class LinagoraEventDetail {
  ...
  const LinagoraEventDetail({
    ...
    this.expansion,
         ...
         valueSpacing >= 0 && valueSpacing < double.infinity,
         'Value spacing must be finite and non-negative',
       ),
       assert(
  ...
  factory LinagoraEventDetail.participants({
    ...
    return LinagoraEventDetail(
      label: labels.label,
      values: organizer == null || organizer.isEmpty
          ? const []
          : [
              ...organizer.values,
...
class LinagoraEventConference {
  ...
  ///
  /// Null keeps the conference component's default English label.
